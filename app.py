...
if st.session_state.page == 'home':
    ...
    with nav_cols[1]: st.button("关于", variant="ghost")
    with nav_cols[2]: st.button("登录", type="secondary")
    ...
    # 居中功能按钮
    btn_col1, btn_col2, btn_col3, btn_col4, btn_col5 = st.columns([1, 1.2, 0.2, 1.2, 1])
    with btn_col2:
        if st.button("🔍 进入语料库 (Corpus Explorer)", use_container_width=True, type="primary"):
            st.session_state.page = 'app'; st.session_state.active_tab = 0; st.rerun()
    with btn_col4:
        if st.button("🤖 在线识别 (Agent AI)", use_container_width=True, type="primary"):
            st.session_state.page = 'app'; st.session_state.active_tab = 1; st.rerun()
    
    # 底部访问统计
    total_visits = get_and_update_visit_count()
    st.markdown(f'<div style="position:fixed; bottom:30px; width:100%; text-align:center; color:#9CA3AF; font-size:0.9rem;">👁️ 累计科研访问量: {total_visits} · © 2026 隐喻计算课题组</div>', unsafe_allow_html=True)
...
# --- B. 功能页 (App) ---
    ...
    with tab1:
        ...
        if not samples: st.warning("未找到语料文件")
            ...
            with c1: search_query = st.text_input("🔍 搜索句子内容")
    ...
    with tab2:
        ...
        if st.button("🚀 启动多智能体分析", type="primary"):
            ...
            with st.status("🕵️‍♂️ Agent 1 (语义)...") as s1:
                p1 = f'这是《{ctx}》中的句子。判定含义并提取可疑比喻词。严格JSON：{{"meaning": "...", "metaphor_words": ["..."]}} 内容: "{ts}"'
