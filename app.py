...
if st.session_state.page == 'home':
    ...
    with col_grid:
        ...
        with r1c1:
            if st.button("🏠 首页", use_container_width=True): pass 
        with r1c2:
            if st.button("ℹ️ 关于平台", use_container_width=True): 
                st.session_state.page = 'about'
                st.rerun()
                
        r2c1, r2c2 = st.columns(2, gap="large")
        with r2c1:
            if st.button("🔍 明清典籍隐喻语料库", use_container_width=True): 
                st.session_state.page = 'corpus'
                st.rerun()
        with r2c2:
            if st.button("🤖 在线隐喻识别", use_container_width=True): 
...
{other_exp_html}
    ...
    # 纯净的原生 Streamlit Columns 布局，不使用任何 HTML 包裹原生组件
