...
def get_model_configs():
    # 核心修复点：使用 .get 防止字典 Key 缺失导致致命崩溃
    try:
        return {
            "Deepseek-V3.2(推荐)": {
                "base_url": "https://api.deepseek.com",
                "model_name": "deepseek-chat",
                "env_key": st.secrets.get("deepseek_api_key", "sk-placeholder")
            },
            "Qwen": {
                "base_url": "https://dashscope.aliyuncs.com/compatible-mode/v1",
                "model_name": "qwen-max",
                "env_key": st.secrets.get("qwen_api_key", "sk-placeholder")
            },
            "GPT-4o": {
                "base_url": "https://openrouter.ai/api/v1",
                "model_name": "openai/gpt-4o-mini",
                "env_key": st.secrets.get("openrouter_api_key", "sk-placeholder")
            }
        }
...
def init_supabase() -> Client:
    try: return create_client(st.secrets.get("SUPABASE_URL", ""), st.secrets.get("SUPABASE_KEY", ""))
    except: return None
            ...
            if run_btn:
                ...
                with st.status("🕵️‍♂️ Agent 1 (语义提取) 正在分析表层结构...", expanded=True) as status1:
                    ...
                    try:
                        ...
                        st.error(f"Agent 1 失败: {e}"); st.stop()
                ...
                with st.status("⚖️ Agent 2 (推理) 正在进行深度隐喻考证...", expanded=True) as status2:
                    ...
                    try:
                        ...
                        st.error(f"Agent 2 失败: {e}"); st.stop()
